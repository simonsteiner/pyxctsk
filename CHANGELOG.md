...
- `optimized_distance`, `optimized_route_coordinates` and `calculate_iteratively_refined_route` keep their signatures (`angle_step` and `beam_width` are now deprecated no-ops) and gained an `earth_model` parameter; `num_iterations` now bounds the alternating sweeps (default `DEFAULT_NUM_ITERATIONS = 100`; convergence normally stops after a handful). New `CONVERGENCE_EPSILON_M = 0.1` in `optimization_config.py`.
