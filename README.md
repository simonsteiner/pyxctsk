...
The library targets the XCTrack Competition Interfaces specification, aiming for compatibility with the XCTrack mobile app and other tools in the paragliding competition ecosystem. Both task formats, manufacturer extensions and all documented fields round-trip byte-identically against the 25 reference tasks generated by tools.xcontest.org. The known gaps — the `XCTSKZ:` compressed QR encoding and validation of the spec's structural rules — are tracked in [docs/spec-conformance/](./docs/spec-conformance/).

## Technical Highlights

- **Typed Data Model**: Core domain objects are dataclasses with constrained values modelled as enums, so unknown task/turnpoint/goal types are rejected at parse time. Note that the spec's structural rules — TAKEOFF only first, SSS and ESS exactly once, SSS before ESS — are not yet enforced ([task.py](./src/pyxctsk/task.py))
