...
The library implements the XCTrack Competition Interfaces specification: both task formats, both QR schemes (`XCTSK:` and the compressed `XCTSKZ:`), manufacturer extensions, and all documented fields. All 25 reference tasks generated by tools.xcontest.org round-trip byte-identically. The conformance review behind that claim, including the design decisions it produced, is in [docs/spec-conformance/](./docs/spec-conformance/).
