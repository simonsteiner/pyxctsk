...
class TestEssentialDistance:
    ...
    def test_single_task_smoke_test(self, test_data_dir):
        ...
        assert results['optimized_distance_km'] > 0, "Optimized distance should be positive"
        assert results['center_distance_km'] > results['optimized_distance_km'], "Optimization should reduce distance"
        assert len(results['turnpoints']) == len(task.turnpoints), "Should have same number of turnpoints"
        
        # Reasonable range checks
        assert 100 <= results['center_distance_km'] <= 200, f"Center distance {results['center_distance_km']:.1f}km seems unreasonable"
        assert 50 <= results['optimized_distance_km'] <= 150, f"Optimized distance {results['optimized_distance_km']:.1f}km seems unreasonable"
        
        print(f"Smoke test passed: {results['center_distance_km']:.1f}km -> {results['optimized_distance_km']:.1f}km")
