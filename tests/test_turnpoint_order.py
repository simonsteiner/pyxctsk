...
def test_turnpoint_field_order():
    ...
    assert keys.index('t') < keys.index('z'), "Type should come before coordinates in ESS"
    ...
    assert sss_keys.index('t') < sss_keys.index('z'), "Type should come before z in SSS turnpoint"
    
    # Check ESS turnpoint
    ess_tp_json = task_dict['t'][1]
    ess_keys = list(ess_tp_json.keys())
    assert ess_keys.index('t') < ess_keys.index('z'), "Type should come before z in ESS turnpoint"
    ...
    for i, tp in enumerate(spec_dict['t']):
        if 't' in tp:
            ...
            if 't' in gen_tp:
                assert gen_keys.index('t') < gen_keys.index('z'), f"Type should come before z in turnpoint {i}"
