...
class TestDistanceAlgorithmThorough:
    ...
    def test_high_precision_calculations(self, test_data_dir):
        ...
        assert 140 <= results['center_distance_km'] <= 160, f"Center distance {results['center_distance_km']:.1f}km seems unreasonable"
        assert 80 <= results['optimized_distance_km'] <= 100, f"Optimized distance {results['optimized_distance_km']:.1f}km seems unreasonable"
        
        # High precision should give good optimization
        savings_percent = (results['center_distance_km'] - results['optimized_distance_km']) / results['center_distance_km'] * 100
        assert savings_percent > 30, f"High precision should give >30% savings, got {savings_percent:.1f}%"
        
        print(f"High precision (2°): {results['optimized_distance_km']:.2f}km ({savings_percent:.1f}% savings)")
