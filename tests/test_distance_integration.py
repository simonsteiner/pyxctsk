...
class TestTaskDistanceIntegration:
    ...
    def test_task_distance_calculations(self, task_file, test_data_dir, expected_results):
        ...
        assert opt_diff <= 2.0, f"Optimized distance diff {opt_diff:.1f}km > 2.0km for {task_file}"  # More relaxed for speed
        ...
        for i, (calculated_tp, expected_tp) in enumerate(zip(results['turnpoints'], expected['turnpoints'])):
            ...
            assert center_tp_diff <= 2.0, f"TP {i+1} center distance diff {center_tp_diff:.1f}km > 2.0km for {task_file}"
            assert opt_tp_diff <= 3.0, f"TP {i+1} optimized distance diff {opt_tp_diff:.1f}km > 3.0km for {task_file}"
    ...
    def test_basic_optimization_effectiveness(self, test_data_dir):
        ...
        assert results['optimized_distance_km'] < results['center_distance_km'], "Optimization failed to reduce distance"
