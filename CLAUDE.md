...
**`tests/test_layering.py` enforces all of that** by parsing the import graph — the layer rules, the sibling-import rule, and the exact set of function-local imports inside the packages. Adding a new one means adding it to `EXPECTED_DEFERRED_IMPORTS` with a reason — both of the two that exist break an import cycle, and only one of them crosses a package boundary. Both rules had already been broken once each while they were only prose, and the second broke `import pyxctsk` outright.
...
- `task_distances.py` — per-leg and cumulative task distances, projected from one `OptimizedRoute` rather than one optimizer run per turnpoint
- `sss.py` — Start-of-Speed-Section entry point / info
- `goal_line.py` — the `GoalLine` deep module: goal-line length, endpoints, semicircular control zone, all on the task's `earthModel` (ADR 0003). It sits here rather than with the KML/GeoJSON writers that draw it because `task_distances` sizes a LINE goal's cylinder from the goal-line length; putting it in `export/` closes an import cycle. **`GoalLine.from_task()` is the only answer to whether a task has a goal line** — `export/common.get_turnpoints_to_render()` derives the render list from it rather than asking again, because the goal line's presence is the whole reason to drop the last turnpoint. A second predicate (`should_skip_last_turnpoint`) used to answer with one clause fewer and a coincident previous turnpoint made the goal disappear from both output formats.
