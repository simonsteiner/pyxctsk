...
class Task(object):
    ...
    def __init__(
        ...
        self.window_open_time = (
            None  # seconds from midnight: takeoff window opening time
        )
        ...
        self.start_iteration = (
            None  # number of start iterations: 0 is indefinite up to start close time
        )
        self.opt_dist = 0
        ...
        self.stats = (
            dict()
        )  # STATIC scored task statistics, used when importing results from JSON / FSDB files
    ...
    def __setattr__(self, attr, value):
        ...
        property_names = [
            p for p in dir(Task) if isinstance(getattr(Task, p), property)
        ]
        if attr == "date":
        ...
        if attr not in property_names:
            ...
            # if (
            #     attr == "task_id"
            #     and hasattr(self, "formula")
            #     and isinstance(self.formula, TaskFormula)
            # ):
    ...
    def date_epoch(self):
        ...
        return (
            ...
            else datetime.combine(self.date, datetime.min.time()).timestamp()
        )
    ...
    def date_utc_epoch(self):
        ...
        return (
            ...
            else datetime.combine(self.date_utc, datetime.min.time()).timestamp()
        )
    ...
    def window_open_time_epoch(self):
        ...
        return (
            ...
            else self.date_epoch + self.window_open_time
        )
    ...
    def window_close_time_epoch(self):
        ...
        return (
            None
            if self.window_close_time is None
            else self.date_epoch + self.window_close_time
        )
    ...
    def start_close_time_epoch(self):
        ...
        return (
            None
            if self.start_close_time is None
            else self.date_epoch + self.start_close_time
        )
    ...
    def task_deadline_epoch(self):
        ...
        return (
            None if self.task_deadline is None else self.date_epoch + self.task_deadline
        )
    ...
    def alt_compensation(self):
        ...
        return (
            0
            if self.formula.scoring_altitude == "GPS"
            else altitude_compensation(self.QNH)
        )
    ...
    def distances_to_go(self):
        ...
        while t >= 1:
            d += distance(
                self.optimised_turnpoints[t], self.optimised_turnpoints[t - 1]
            )
    ...
    def duration(self):
        ...
        if self.comp_class == "PG":
            ...
            """
            if (
                self.task_type == "elapsed time" or self.SS_interval
            ) and self.max_ss_time:
    ...
    def ftv_validity(self):
        ...
        if self.formula.overall_validity == "ftv":
            return c_round(
                (
                    self.max_score / 1000
                    if self.formula.validity_ref == "max_score"
                    else self.day_quality
                ),
                4,
            )
        else:
        ...
        if self.task_type == "race" and self.SS_interval:
            if self.start_iteration:
                ...
                if self.start_close_time:
                    return int(
                        (self.start_close_time - self.start_time) / self.SS_interval
                    )
    ...
    def valid_results(self):
        return [
            ...
            if pilot.result_type not in ("abs", "dnf", "nyp")
        ]
    ...
    def results_landed_out(self):
        return [
            pilot for pilot in self.valid_results if not pilot.result_type == "goal"
        ]
    ...
    def tot_dist_flown(self):
        if self.formula:
            if self.formula.min_dist and self.pilots_launched > 0:
                return sum(
                    [p.distance_flown for p in self.valid_results if p.distance_flown]
                )
        return 0
    ...
    def tot_dist_over_min(self):
        if self.formula:
            if self.formula.min_dist and self.pilots_launched > 0:
                return sum(
                    [
                        max(p.distance_flown - self.formula.min_dist, 0)
    ...
    def std_dev_dist(self):
        ...
        try:
            return stdev(
                [
                    max(p.distance_flown, self.formula.min_dist)
                    for p in self.valid_results
                ]
            )
    ...
    def max_distance_flown(self):
        if self.formula:
            return max(
                max((p.distance_flown for p in self.valid_results), default=0),
                self.formula.min_dist or 0,
            )
        return 0
    ...
    def max_distance(self):
        if self.formula:
            ...
            return max(
                max((p.distance for p in self.valid_results), default=0),
                self.formula.min_dist or 0,
            )
        return 0
    ...
    def min_dept_time(self):
        return min(
            (p.real_start_time for p in self.results_with_SSS_time), default=None
        )
    ...
    def last_landing_time(self):
        ...
        return max(
            ((p.landing_time or p.last_time or 0) for p in self.valid_results),
            default=None,
        )
    ...
    def last_landout_time(self):
        ...
        return max(
            (
                (p.landing_time or p.last_time or 0)
                for p in self.valid_results
                if not p.ESS_time
            ),
            default=None,
    ...
    def max_time(self):
        if self.pilots_launched:
            last_pilot_time = (
                self.last_landout_time
                if not self.max_ess_time
    ...
    def max_score(self):
        return max(
            (p.score for p in self.valid_results if p.score is not None), default=None
        )
    ...
    #         self.task_path = "_".join(
    #             [("t" + str(self.task_num)), self.date.strftime("%Y%m%d")]
    #         )
    #     else:
    #         return
    #     if self.id:
    ...
    #         airspace = (
    #             None if not self.airspace_check else AirspaceCheck.from_task(self)
    #         )
    ...
    #     pil_list = sorted(
    #         [p for p in self.pilots if p.result_type not in ["dnf", "abs", "nyp"]],
    ...
    #     formula = {
    #         x: getattr(self.formula, x)
    #         for x in TaskResult.formula_list
    #         if x in dir(self.formula)
    #     }
    ...
    #         results.append(res)
    #     rankings = None  # create_rankings(self.comp_id, self.comp_class)
    ...
    def is_valid(self):
        ...
        if self.pilots_launched == 0:
            print(
                f"Tracks (Task ID {self.id}) have not been checked yet or stats have not been updated."
            )
            return False
        if not self.stopped_time:
            return True
    ...
    def create_projection(self):
        ...
        # Use GAP2025 default values if formula values aren't set
        # GAP2025 defaults:
        # - tolerance: 0.1% (0.001)
        # - min_tolerance: 5 meters
        gap2025_tolerance = 0.001  # 0.1%
        gap2025_min_tolerance = 5  # meters

        # Get values from formula with fallbacks to GAP2025 defaults
        tol = getattr(self.formula, "tolerance", None) or gap2025_tolerance
        min_tol = getattr(self.formula, "min_tolerance", None) or gap2025_min_tolerance
    ...
    def update_from_dict(task_id: int, data: dict):
        ...
        for key, value in data.items():
            if hasattr(task, key):
                if task.time_offset != 0 and any(
                    s in key for s in ("_time", "_deadline")
                ):
        ...
        for idx, tp in enumerate(data["route"], 1):
            ...
            turnpoint = Turnpoint(
                tp["lat"], tp["lon"], tp["radius"], tp["type"], tp["shape"], tp["how"]
            )
    ...
    #     if not t:
    #         print(
    #             f"There's no active json file for task {task_id}, or given filename does not exists"
    #         )
    #         return None
    ...
    #     taskpoints = task.getElementsByTagName("taskpoints")[
    #         0
    ...
    #         turnpoint = Turnpoint(
    #             lat=lat, lon=lon, radius=radius, type=type, shape=shape, how=how
    #         )
    ...
    #     task.comment = (
    #         state
    #         if not node.get("cancel_reason")
    #         else ": ".join([state, node.get("cancel_reason")])
    #     )
    #     if state == "CANCELLED":
    ...
    #     elif state == "STOPPED":
    #         task.stopped_time = (
    #             time_to_seconds(get_time(node.get("stop_time"))) - offset
    #         )
    ...
    #     task.opt_dist_to_ESS = (
    #         None if p is None else float(p.get("launch_to_ess_distance")) * 1000
    #     )
    ...
    #         task.dist_validity = (
    #             0 if p is None else float(p.get("distance_validity") or 0)
    #         )
    #         task.time_validity = 0 if p is None else float(p.get("time_validity") or 0)
    #         task.launch_validity = (
    #             0 if p is None else float(p.get("launch_validity") or 0)
    #         )
    ...
    #         task.avail_dist_points = (
    #             0 if p is None else float(p.get("available_points_distance") or 0)
    #         )
    #         task.avail_dep_points = (
    #             0 if p is None else float(p.get("available_points_leading") or 0)
    #         )
    #         task.avail_time_points = (
    #             0 if p is None else float(p.get("available_points_time") or 0)
    #         )
    #         task.avail_arr_points = (
    #             0 if p is None else float(p.get("available_points_arrival") or 0)
    #         )
    ...
    #         if node is None
    #         else float(
    #             node.get("qnh_setting").replace(",", ".")
    #             if node.get("qnh_setting")
    #             else 1013.25
    #         )
    #     )
    ...
    #             task.task_type = "race"
    #             task.start_time = (
    #                 time_to_seconds(get_time(node.find("FsStartGate").get("open")))
    #                 - offset
    #             )
    #             if startgates > 1:
    ...
    #                 time = (
    #                     time_to_seconds(
    #                         get_time(node.findall("FsStartGate")[1].get("open"))
    #                     )
    #                     - offset
    #                 )
    ...
    #                 """launch"""
    #                 turnpoint = Turnpoint(
    #                     float(w.get("lat")), float(w.get("lon")), int(w.get("radius"))
    #                 )
    #                 turnpoint.name = w.get("id")
    #                 turnpoint.num = len(turnpoints) + 1
    #                 turnpoint.type = "launch"
    #                 task.date = get_date(w.get("open"))
    #                 task.window_open_time = (
    #                     time_to_seconds(get_time(w.get("open"))) - offset
    #                 )
    #                 task.window_close_time = (
    #                     time_to_seconds(get_time(w.get("close"))) - offset
    #                 )
    ...
    def calculate_optimised_task_length(self, method="fast_andoyer"):
        ...
        if self.has_goal_line and self.has_optimised_line:
            ...
            line = convert_turnpoints(
                get_line(
                    self.turnpoints,
                    self.optimised_turnpoints,
                    self.formula.tolerance,
                    self.formula.min_tolerance,
                    True,
                ),
                self.geo,
            )
        ...
        if (
            not self.has_ess
            or not hasattr(self.formula, "ss_dist_calc")
            or self.formula.ss_dist_calc in (None, "launch_to_goal")
        ):
            self.SS_distance = sum(self.optimised_legs[sss_wpt:ess_wpt])
        else:
            optimised = get_shortest_path(self, ss_distance=True)
            optimised_legs = [0] + [
                distance(e, optimised[i + 1], method)
                for i, e in enumerate(optimised[:-1])
            ]
    ...
    #             opt_lat, opt_lon = (
    #                 self.optimised_turnpoints[idx].lat,
    #                 self.optimised_turnpoints[idx].lon,
    #             )
    ...
    #                 next(
    #                     tp for tp in self.turnpoints if tp.num == elem["num"]
    #                 ).wpt_id = elem["wpt_id"]
    #         db.commit()
    ...
    #         print(f"*** Cicle {i}:")
    #         print(
    #             f" -- Getting livetracks starting from time {max(p.last_time for p in pilots)} ..."
    #         )
    #         previous = response
    ...
    #             if (
    #                 p.livetrack
    #                 and p.livetrack[-2].rawtime > (p.last_time or 0)
    #                 and not p.goal_time
    #             ):
    ...
    #         task_coords.append(
    #             {"longitude": obj.lon, "latitude": obj.lat, "name": obj.name}
    #         )
...
#             db.query(TW).filter(TW.track_id.in_(track_list)).delete(
#                 synchronize_session=False
#             )
#             db.query(N).filter(N.track_id.in_(track_list)).delete(
#                 synchronize_session=False
#             )
...
#         min_tolerance = (
#             data.get("min_tolerance") or 5
#         )  # this could be missing in old files
#         bbox = data["bbox"]
...
#         if task:
#             formula_last_update = (
#                 db.query(F.last_update).filter_by(comp_id=task.comp_id).scalar()
#             )
#             tracks = (
#                 db.query(R.last_update, R.result_type).filter_by(task_id=task_id).all()
#             )
#             min_track_update = min(
#                 t.last_update
#                 for t in tracks
#                 if t.result_type not in ["nyp", "abs", "dnf"]
#             )
...
#     with db_session() as db:
#         row = (
#             db.query(TaskObjectView.task_path, TaskObjectView.comp_path)
#             .filter_by(task_id=task_id)
#             .one()
#         )
#         return (
#             None
#             if not (row.task_path and row.comp_path)
#             else Path(TRACKDIR, row.comp_path, row.task_path)
#         )
