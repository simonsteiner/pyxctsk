...
def create_arc_polygon(center, start, end, clockwise=True, tolerance=5):
    ...
    bearing1 = calcBearing(
        center.latitude, center.longitude, start.latitude, start.longitude
    )
    dist1 = geodesic(center, start).meters
    bearing2 = calcBearing(
        center.latitude, center.longitude, end.latitude, end.longitude
    )
    ...
    for i in range(1, points):
        pt = distance.distance(meters=dist1 + dr * i).destination(
            center, bearing1 + da * i
        )
...
def calculate_min_points(angle, radius, tolerance=5):
    ...
    return (
        math.ceil(math.radians(angle) / (2 * math.acos(1 - tolerance / radius)) + 1) * 2
    )
