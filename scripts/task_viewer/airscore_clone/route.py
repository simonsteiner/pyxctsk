...
class Turnpoint:
    ...
    def __init__(
        ...
        assert shape in ["line", "circle", "optimised"], (
            "turnpoint shape is not valid: %r" % shape
        )
        assert how in ["entry", "exit", "optimised"], (
            "turnpoint how (direction) is not valid: %r" % how
        )
...
def cartesian2polar(xyz):
    ...
    flat = math.asin(
        math.sqrt(
            xyz[2] * xyz[2] / ((1 - eSq) * a * (1 - eSq) * a + xyz[2] * xyz[2] * eSq)
        )
    )
...
def start_made_civl(fix, next, start, tolerance, min_tol_m):
    ...
    if start.how == "entry":
        ...
        condition = not (
            start.in_radius(fix, -tolerance, -min_tol_m)
        ) and start.in_radius(next, tolerance, min_tol_m)
        # print(f"how: {start.how} (entry) | dist: {distance(start, fix)} | made: {condition}")
    else:
        """exit start cylinder"""
        condition = start.in_radius(fix, tolerance, min_tol_m) and not (
            start.in_radius(next, -tolerance, -min_tol_m)
        )
...
def tp_made_civl(fix, next, tp, tolerance, min_tol_m):
    ...
    condition = (
        not tp.in_radius(fix, -tolerance, -min_tol_m)
        and tp.in_radius(next, tolerance, min_tol_m)
    ) or (
        tp.in_radius(fix, tolerance, min_tol_m)
        and not tp.in_radius(next, -tolerance, -min_tol_m)
    )
    return condition
...
def in_semicircle(wpts, idx, fix, t=0.001, min_t=5):
    ...
    if wpt.in_radius(fix, t, min_t):
        fcoords = namedtuple(
            "fcoords", "flat flon"
        )  # wpts don't have flon/flat so make them for polar2cartesian
        ...
        B = polar2cartesian(
            Turnpoint(lat=geo_b.latitude, lon=geo_b.longitude)
        )  # initialized
...
def opt_goal(p1, p2):
    if p2.shape == "line":
        ...
        p = geod.Direct(
            p2.lat, p2.lon, calcBearing(p2.lat, p2.lon, p1.lat, p1.lon), p2.radius
        )
        return Turnpoint(
            lat=p["lat2"],
            lon=p["lon2"],
...
def opt_wp_exit(opt, t1, exit):
    ...
    while not found:
        ...
        dist_from_centre = int(
            c_round(geod.Inverse(point.lat, point.lon, exit.lat, exit.lon)["s12"])
        )
...
def opt_wp_enter(opt, t1, enter):
    ...
    while not found:
        ...
        dist_from_centre = int(
            c_round(geod.Inverse(point.lat, point.lon, enter.lat, enter.lon)["s12"])
        )
...
def get_fix_dist_to_goal(task, fix, pointer) -> tuple:
    ...
    if ESS_index:
        dist_to_ESS = sum(
            hypot(e.fx - points[i + 1].fx, e.fy - points[i + 1].fy)
            for i, e in enumerate(points[:ESS_index])
        )
...
def calculate_optimised_path(points: list, ESS_index: int or None, line: list) -> tuple:
    ...
    tolerance = (
        1.0  # meters, difference between results under which iteration will stop
    )
...
def optimize_path(
    points: list, count: int, ESS_index: int or None, line: list
) -> float:
    """Inputs:
...
def get_target_points(
    points: list, count: int, index: int, ESS_index: int or None
) -> tuple:
    """Inputs:
    points  - array of point objects
...
def process_cylinder(c, a, b):
    ...
    if distAB == 0.0:
        ...
        if distAC < c.radius and distBC < c.radius:
            ...
            distAC < c.radius
            and distBC > c.radius
            or (distAC > c.radius and distBC < c.radius)
        ):
