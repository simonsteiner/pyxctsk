...
def get_isotime(d: datetime.date, t: int, offset=None):
    ...
    return dd.combine(
        d, sec_to_time(t + offset), tzinfo=dt.timezone(offset=tz)
    ).isoformat()
...
def altitude_compensation(QNH: float):
    ...
    if math.isclose(QNH, 1013.25, abs_tol=100) and not math.isclose(
        QNH, 1013.25, abs_tol=0.01
    ):
...
def get_season_dates(
    ...
    starts = datetime.strptime(
        f"{s}-{date_from.month}-{date_from.day}", "%Y-%m-%d"
    ).date()
...
def CalculateQnhAltitude(pressure: float, qnh: float):
    ...
    if not pressure or not qnh or qnh < 900.0 or qnh > 1100.0:
        ...
        return (
            288.15
            / 0.0065
            * (1 - math.exp(math.log(pressure / qnh) * 0.190266669078492))
        )
...
def CalculatePressure(baroAltitude: float):
    ...
    return 1013.25 * math.exp(
        5.25578129287301 * math.log(1 - 0.0065 * baroAltitude / 288.15)
    )
