...
class GNSSFix:
    ...
    def build_from_B_record(B_record_line, index):
        ...
        return GNSSFix(rawtime, lat, lon, validity, press_alt, gnss_alt, index, extras)

    def __init__(self, rawtime, lat, lon, validity, press_alt, gnss_alt, index, extras):
    ...
    def _compute_bearing_change_rates(self):
        ...
        for curr_fix in range(len(self.fixes)):
            ...
            if prev_fix is None:
                ...
                bearing_change = (
                    self.fixes[prev_fix].bearing - self.fixes[curr_fix].bearing
                )
                if math.fabs(bearing_change) > 180.0:
                    if bearing_change < 0.0:
                        ...
                        bearing_change -= 360.0
                time_change = (
                    self.fixes[prev_fix].timestamp - self.fixes[curr_fix].timestamp
                )
