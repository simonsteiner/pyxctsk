...
def _compute_optimal_route_dp(turnpoints: List[TaskTurnpoint], task_turnpoints=None, angle_step: int = DEFAULT_ANGLE_STEP, 
                             show_progress: bool = False, return_path: bool = False) -> Tuple[float, List[Tuple[float, float]]]:
    ...
    if len(turnpoints) < 2:
        distance = 0.0
        path = [(tp.center[0], tp.center[1]) for tp in turnpoints] if return_path else []
    ...
    for i in range(1, len(turnpoints)):
        ...
        for curr_pt in perimeters[i]:
            ...
            if return_path and sss_index is not None and first_tp_after_sss_index is not None:
...
def optimized_distance(turnpoints: List[TaskTurnpoint], angle_step: int = DEFAULT_ANGLE_STEP, show_progress: bool = False) -> float:
    ...
    """
    distance, _ = _compute_optimal_route_dp(turnpoints, angle_step=angle_step, show_progress=show_progress, return_path=False)
    return distance
...
def calculate_task_distances(task: Task, angle_step: int = DEFAULT_ANGLE_STEP, show_progress: bool = False) -> Dict[str, Any]:
    ...
    for i, tp in enumerate(task.turnpoints):
        if hasattr(tp, 'type') and tp.type and tp.type.value == 'SSS':
            ...
            if i + 1 < len(task.turnpoints):
                ...
                distance_turnpoints = [turnpoints[0]] + turnpoints[first_tp_after_sss_index:]
            break
    
    if show_progress:
        if sss_index is not None:
            print(f"  📏 SSS task detected - calculating distance from takeoff to first TP after SSS ({len(distance_turnpoints)} turnpoints)...")
        else:
    ...
    opt_dist = optimized_distance(distance_turnpoints, angle_step=angle_step, show_progress=show_progress)
    ...
    for i, (tp, task_tp) in enumerate(zip(task.turnpoints, turnpoints)):
        ...
        if sss_index is not None and first_tp_after_sss_index is not None:
            if i <= sss_index:
                ...
                leg_distance = geodesic(takeoff_tp.center, task_tp.center).meters / 1000.0
                cumulative_center = leg_distance
                
                # For optimized, use the actual distance calculation
                partial_distance_turnpoints = [distance_turnpoints[0], distance_turnpoints[1]]  # takeoff + first after SSS
                cumulative_opt = optimized_distance(partial_distance_turnpoints, angle_step=angle_step, show_progress=False) / 1000.0
            else:
                ...
                distance_tp_index = i - sss_index  # Adjust index for distance_turnpoints list
                if distance_tp_index < len(distance_turnpoints):
                    partial_distance_turnpoints = distance_turnpoints[:distance_tp_index+1]
...
def optimized_route_coordinates(turnpoints: List[TaskTurnpoint], task_turnpoints=None, angle_step: int = DEFAULT_ANGLE_STEP) -> List[Tuple[float, float]]:
        ...
        for i, tp in enumerate(task_turnpoints):
            if hasattr(tp, 'type') and tp.type and tp.type.value == 'SSS':
                if i + 1 < len(task_turnpoints):
                    ...
                    route_turnpoints = [turnpoints[0]] + turnpoints[first_tp_after_sss_index:]
...
def calculate_optimal_sss_entry_point(
    ...
    best_sss_point = min(sss_perimeter, 
                        key=lambda p: geodesic(takeoff_center, p).meters + geodesic(p, first_tp_after_sss_point).meters)
...
def calculate_sss_info(
    ...
    for i, tp in enumerate(task_turnpoints):
        if hasattr(tp, 'type') and tp.type and tp.type.value == 'SSS':
            # Found SSS turnpoint
            sss_tp = {'lat': tp.waypoint.lat, 'lon': tp.waypoint.lon, 'radius': tp.radius}
            ...
            if i + 1 < len(task_turnpoints):
                ...
                if len(route_coordinates) > 1:
                    ...
                    first_tp_after_sss_route_point = (first_tp_after_sss['lat'], first_tp_after_sss['lon'])
                ...
                return {
                    ...
                    'takeoff_center': {'lat': takeoff_center[0], 'lon': takeoff_center[1]}
                }
